...
type Service struct {
	...
	// Optional pair-flow collaborators (set by WithProvisioning); the
	// binding and listing operations work without them.
...
// stores + encryptService that PersistFleetNodePairResult uses to create the
// device, credentials, and pairing records, and the dispatcher PairOnNode uses
// to send pair commands over a node's ControlStream. Returns the service for
// chaining.
...
// pairDeviceLocked binds a device to a fleet node within an existing
// transaction: it locks the node, refuses cloud-dialed devices, inserts the
// fleet_node_device row, and transfers discovery attribution. The caller owns
// the transaction (PairDevice and PersistFleetNodePairResult both wrap it).
func (s *Service) pairDeviceLocked(ctx context.Context, fleetNodeID, deviceID, orgID int64, assignedBy *int64) error {
	// Lock-and-recheck inside the TX so a concurrent revoke can't soft-delete
	// the node between the status check and the INSERT. Matches the lock order
	// Confirm/Revoke use.
	...
	// Refuse a device the cloud actively dials (cloud-paired): the discovery
	// upsert guard blocks refreshing a cloud-paired row, so pairing it here would
	// leave the node unable to refresh while the API reports it as fleet-node
	// paired. Operator must unpair from the cloud first.
	...
	// Make the paired node the discovery owner so its future reports refresh the
	// row instead of being rejected by the upsert's attribution guard (e.g. after
	// replacing a revoked node). No-op for devices with no discovered_device origin.
