...
func TestRegistry_AdmitAndScopePairResults_RejectsEmptyAndQuotaAndKind(t *testing.T) {
	// Arrange
	r := NewRegistry()
	pair := &PairMeta{OrgID: 1, Targets: map[string]struct{}{"a": {}}} // maxReports = 1
	...
	// More results than the dispatched target count -> quota exceeded.
	_, _, err = r.AdmitAndScopePairResults(4, "p", []*gatewaypb.FleetNodePairResult{
		{DeviceIdentifier: "a"}, {DeviceIdentifier: "b"},
	})
	assert.ErrorIs(t, err, ErrReportQuotaExceeded)
